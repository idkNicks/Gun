...
import io.github.daawn.gun.ShootSound;
...
public class Marshall extends Gun {
    ...
    public List<ShootEffect> shootEffects() {
        return List.of(
                new TrailEffect(Particle.CRIT_MAGIC, 30, 8.0, 30.0),
                new TrailEffect(Particle.ASH, 3, 5.0, 30.0),
    ...
    protected List<ShootSound> shootSounds() {
        return List.of(
                ...
                new ShootSound(Sound.ENTITY_GENERIC_EXPLODE, 1F, 3F)
        );
    }

    @Override
    protected ItemStack createGunItem() {
        return new ItemBuilder(Material.IRON_HORSE_ARMOR)
                .setName("마샬")
                .build();
    }
}